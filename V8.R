...
pause_viewer(historique_guide)
    ...
    ### 2.5.3 Tests indicateurs et type - ANALYSE KRUSKAL-WALLIS & DUNN ----

### 2.5 ANALYSE STATISTIQUE DE BASE (KRUSKAL-WALLIS) ----
library(tidyverse)

# 1. Préparation des données (Assurez-vous que vos dataframes sources sont bien définis)
# Fusion pour avoir stade + variables numériques
df_stats_test <- data_afc_detail %>%
  left_join(data_types_calcules, by = "id_unique_q") %>%
  left_join(data_num_sync %>% rownames_to_column("id_unique_q"), by = "id_unique_q") %>%
  filter(!is.na(stade))

# Liste des variables à tester
vars_test <- c(names(tableau_types_bio), names(data_num_sync))

# 2. Exécution du test de Kruskal-Wallis
results_kw <- lapply(vars_test, function(var) {
  test <- kruskal.test(get(var) ~ as.factor(stade), data = df_stats_test)
  return(data.frame(
    Variable = var,
...
tab_kw <- do.call(rbind, results_kw)
tab_kw <- tab_kw %>%
...
# Affichage simple (Console ou Viewer)
print(tab_kw)

### 2.5.4 TEST DE DUNN ET SYNTHÈSE AUTO ----
library(dunn.test)
library(knitr)
library(kableExtra)

# 1. Nettoyage strict : on garde uniquement les colonnes nécessaires
# On s'assure qu'il n'y a pas de NA dans le stade
df_dunn <- df_stats_test %>% 
  filter(!is.na(stade)) %>% 
  mutate(stade = as.factor(stade))

# 2. Boucle de test (sans gestion complexe d'erreurs pour éviter les conflits)
res_dunn_list <- list()

for(var in vars_test) {
  # On extrait les données pour cette variable
  valeurs <- df_dunn[[var]]
  stades <- df_dunn$stade
  
  # Test de Dunn simple
  dunn_res <- dunn.test(valeurs, stades, method="bh", kw=FALSE, label=FALSE)
  
  # Extraction des p-values ajustées
  p_adj <- dunn_res$P.adjusted
  
  # Conversion en symboles
  syms <- symnum(p_adj, cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", "ns"))
  
  res_dunn_list[[var]] <- data.frame(
    Variable = var,
    `À restau - Restauré` = as.character(syms[1]),
    `À restau - Témoin`   = as.character(syms[2]),
    `Restauré - Témoin`   = as.character(syms[3]),
    check.names = FALSE
  )
}

# 3. Création et affichage du tableau final
table_final_dunn <- do.call(rbind, res_dunn_list)

table_final_dunn %>%
  kable(format = "html", caption = "Synthèse des différences significatives (Test de Dunn)") %>%
